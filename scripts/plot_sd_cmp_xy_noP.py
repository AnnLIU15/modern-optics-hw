...
def plot_sd(csv_path: Union[str,List[str]],
    ...
    (fig,axes), _, XYZ_1931, _ = get_background(type_xyz=0,is_plot_white=False)
    (fig1,_), _, XYZ_2006, _ = get_background(type_xyz=1,is_plot_white=False)
    plt.close(fig1)
    color_list = ['#333333','#99402B','#4B1CE0','#663333','#AD36CC']
    
    spd_data,file_str,wavelengths = get_spd(csv_path,spd_type=0,if_sep=True)
    ...
    for idx,file_name in enumerate(file_str):
        for idx_xyz in range(2):
            XYZ = XYZ_2006 if idx_xyz else XYZ_1931
            ...
            axes.plot(new_xy[..., 0], new_xy[..., 1],
                    '-.' if idx_xyz else ':',color=color_list[idx],
                    label=file_str[idx]+f'({"2006" if idx_xyz else "1931"},NTSC {Percent}%)')
    
    axes.set_title(f'Chromatic Diagram with CIE 1931-XYZ')
    ...
    if is_save:
        plt.savefig(f'{base_dir}/{file_name}-cmparea-sd.pdf', format = 'pdf',
                bbox_inches='tight',pad_inches = 0,transparent = True)
        plt.savefig(f'{base_dir}/{file_name}-cmparea-sd.svg', format = 'svg',
                    bbox_inches='tight',pad_inches = 0,transparent = True)
        plt.savefig(f'{base_dir}/{file_name}-cmparea-sd.png', format = 'png', dpi=300,
                    bbox_inches='tight',pad_inches = 0,transparent = True)
    if is_show:
        plt.show()
    plt.close('all')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--files',type=str,nargs='+',default= ['data/psu_llab/CQS-1-NM.csv','data/OSCONIQ P2226/P2226.csv','data/OSCONIQ P3030/P3030.csv'],
    ...
    parser.add_argument('--base_dir',default='outputs/cmp/')
