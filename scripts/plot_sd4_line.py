...
def plot_sd(csv_path: Union[str,List[str]],
    ...
    for idx,file_name in enumerate(file_str):
        ...
        #     y = xy_move_data[:,1]
        #     y_hat = func(x)
        #     pos = np.argmin(y-y_hat)
        #     if pos < left_move_idx:
        #         # 380 + pos -> halfpos_left
        #         cur_wave_center = blue_wave_length + (380 + pos) - half_power[0]
        #     else:
        #         cur_wave_center = blue_wave_length + 1 + (pos - left_move_idx)
        #     axes.scatter(xy_move_data[pos,0],xy_move_data[pos,1],color='#FF0000',marker='+',s=10)
    ...
    if is_save:
        plt.savefig(f'{base_dir}/{file_name}-4.pdf', format = 'pdf',
                bbox_inches='tight',pad_inches = 0,transparent = True)
        plt.savefig(f'{base_dir}/{file_name}-4.svg', format = 'svg',
                    bbox_inches='tight',pad_inches = 0,transparent = True)
        plt.savefig(f'{base_dir}/{file_name}-4.png', format = 'png', dpi=300,
                    bbox_inches='tight',pad_inches = 0,transparent = True)
    if is_show:
        plt.show()
    plt.close('all')
